...
export class AgregarvehiculoComponent implements OnInit {
  ...
  guardar() {
    ...
    vehiculo.tipoVehiuclo = this.form.value['capacidad'];
    vehiculo.capacidad = this.form.value['tipoVehiculo'];
    ...
    if (this.edicion === true) {
      ...
      this.vehiculoService.editar(vehiculo).subscribe(() => {
        ...
        this.router.navigate(['/inicio/vehiculo']);
      });
    } else {
      this.vehiculoService.guardar(vehiculo).subscribe(() => {
        this.form.reset();
        this.vehiculoService.mensajeCambio.next('Vehículo guardado satisfactoreamente');
        this.router.navigate(['/inicio/vehiculo']);
      });
    }
